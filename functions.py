...
class InventorySystemInfinite(InventorySystemBase):
    ...
    def value_iteration(self):
        ...
        epsilon = 1e-1 #can be adjusted
        gamma = 0.9  #can be adjusted
        i = 0
        while i < 100:
            i += 1
            print('\n i =', i)
            ...
            for s in self.S:
                ...
                for a in A:
                    Q[a] = self.C(s, a) + gamma * sum(self.P(s_next, s, a) * oldV[s_next] for s_next in self.S)
                V[s] = max(Q.values())
                optimal_policy[s] = max(Q, key=Q.get)
            if all(np.linalg.norm(oldV[s] - V[s]) < epsilon*(1-gamma)/(2*gamma) for s in self.S) or i == 100:
                break
