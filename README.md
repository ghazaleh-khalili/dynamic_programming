...
At the beginning of each period, the DM observes the inventory level, which is represented as the state variable $S_t$, where $t$ is the period index.
...
```math
V_t\left(S_t\right)={min}_{a_t\in A_t}{C_t\left(S_t,a_t\right)+\gamma\sum_{s^\prime\epsilon S}{P\left(S_{t+1}=\left.\ s^\prime\right|S_t=s,a_t=a\right)V_{t+1}\left(s^\prime\right)}}   \quad    \forall\ t=0,\cdots,T 
```
...
This algorithm solves finite-horizon problems by employing backward dynamic programming. It starts from the final period and calculates the cost function of dynamic programming (DP). Then, it moves backward and computes the cost of the previous period. This procedure continues until the first period.

In other words, when we are at time 𝑡 and state 𝑆𝑡, the value of 𝑉𝑡+1(𝑆𝑡) has already been computed. Therefore, by finding the total cost of 𝑆𝑡, we can calculate the 𝑉𝑡(𝑆𝑡) defined in equation (3). This algorithm is commonly known as backward dynamic programming.

Implementing the backward dynamic programming algorithm for finite-horizon problems is straightforward. The steps of the algorithm are as follows:
...
![Algorithm 1 Pseudocode](path/to/algorithm1.png)

### Algorithm 2: Infinite-horizon Problems

The infinite-horizon problem differs slightly from the finite-horizon problem. In this case, since the last period is unknown, we cannot directly apply the algorithms used for finite-horizon problems. Instead, we utilize an iterative algorithm called value iteration to update the cost function of dynamic programming (DP) until a stopping criterion is met.

Value iteration operates by iteratively updating the cost function until the expected cost function converges within a specified error tolerance. The stopping criterion is typically defined using a gap measure, such as 𝜖(1−𝛾)2𝛾, where 𝜖 represents the desired error tolerance and 𝛾 is the discount factor.

It is worth noting that the infinite-horizon problem can be thought of as a steady-state problem, providing a better understanding of the case.
...
![Algorithm 2 Pseudocode](path/to/algorithm2.png)
...
4. Open the `main.py` file to view and execute the main code that solves the inventory problem using dynamic programming.
...
The `main.py` file contains the execution code for solving the inventory problem using dynamic programming. It utilizes the value iteration algorithm for the infinite-horizon problem and the backward dynamic programming algorithm for the finite-horizon problem. The problem assumes that demand follows a Poisson distribution.
...
5. Perform backward dynamic programming on the finite-horizon problem using the `backward_DP()` method of the `InventorySystemFinite` class. This calculates the optimal policy and value function for the specified time horizon.
...
The `function.py` file contains the implementation of the Inventory Problem using dynamic programming. It includes three classes: `InventorySystemBase`, `InventorySystemInfinite`, and `InventorySystemFinite`.
...
The `InventorySystemFinite` class extends the `InventorySystemBase` class and represents an inventory system for finite-horizon problems. It uses the backward dynamic programming algorithm to solve the problem. Key methods include:

- `backward_DP(T)`: Performs the backward dynamic programming algorithm to determine the optimal policy and value functions for a given time horizon `T`.
